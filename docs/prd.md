...
4. **Autenticación OAuth2/JWT (Resource Server) con mapeo de roles Keycloak:** El servicio valida el JWT de forma stateless contra Keycloak (`issuer-uri` → descubrimiento JWKS), sin introspección por request. Un `JwtAuthenticationConverter` personalizado combina los scopes (`SCOPE_*`) con los roles del realm (`realm_access.roles` → `ROLE_*`), habilitando `hasRole("ADMIN")` en las rutas de escritura. Las lecturas de catálogo son públicas. Esta validación distribuida (defensa en profundidad) es compatible con un API Gateway tipo Spring Cloud Gateway: el gateway puede enfocarse en routing, CORS y rate-limiting, reenviando el token con `TokenRelay` sin necesidad de re-autenticar, y el servicio no requiere que Keycloak se exponga vía gateway (solo necesita acceso al `issuer-uri` para JWKS). El prefijo de ruta `/api/v1` debe preservarse en el gateway.
